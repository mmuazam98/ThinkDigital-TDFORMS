(function ($) {
  ...
  $.fn.JPaging = function (param) {
    ...
    if ($pageCount >= 1 && $visiblePageSize >= 1) {
      $("#paging").append("<a href='javascript:void(0)' style='font-weight:700;'>" + "<");
      ...
      if ($pageCount > $visiblePageSize) {
        $("#paging").append("<a href='javascript:void(0)' id='next_point'>" + "...");
      }
      $("#paging").append("<a href='javascript:void(0)' style='font-weight:700;'>" + ">");
    ...
    $("#pre_point").on("click", function (event) {
      ...
      var prevIndex = $(this).nextAll("a:not('.hidden,#next_point')").first().index();
    ...
    $("#paging").on("click", "a:not('#pre_point,#next_point')", function () {
      ...
      if ($(this).is("#paging a:first") === true) {
        ...
        $("#paging a").removeClass("aktif");
        $("#paging a:not('#next_point'):eq(" + $currentIndex + ")").addClass("aktif");
        $("#paging a:not('#next_point'):eq(" + $currentIndex + ")").removeClass("hidden");
      ...
      if ($(this).is("#paging a:last") === true) {
        ...
        if ($currentIndex - 1 > $visiblePageSize && $("#paging a.hidden").length >= 1) {
